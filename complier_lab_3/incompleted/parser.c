...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      Object* obj = createConstantObject(lookAhead->string);
      eat(TK_IDENT);
      eat(SB_EQ);
...
void compileBlock2(void) {
  ...
  if (lookAhead->tokenType == KW_TYPE) {
    ...
    do {
      Object* obj = createTypeObject(lookAhead->string);
      eat(TK_IDENT);
      eat(SB_EQ);
...
void compileBlock3(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      Object* obj = createVariableObject(lookAhead->string);
      eat(TK_IDENT);
      eat(SB_COLON);
      Type* type = compileType();
      eat(SB_SEMICOLON);
...
void compileFuncDecl(void) {
  ...
  func = createFunctionObject(lookAhead->string);
  eat(TK_IDENT);
  declareObject(func);
...
void compileProcDecl(void) {
  ...
  Object* proc;  
  eat(KW_PROCEDURE);  
  proc = createProcedureObject(lookAhead->string);
  eat(TK_IDENT);
  declareObject(proc);
...
void compileParam(void) {
  ...
  param = createParameterObject(lookAhead->string, paramKind, owner);
  eat(TK_IDENT);
  eat(SB_COLON);
